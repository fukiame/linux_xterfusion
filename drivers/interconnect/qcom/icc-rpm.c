...
static int qcom_icc_rpm_set(struct qcom_icc_node *qn, u64 *bw, bool ignore_enxio, u64 *applied_bw)
{
	int ret, rpm_ctx = 0;
	u64 bw_bps;
...
static int qcom_icc_set(struct icc_node *src, struct icc_node *dst)
{
	...
	ret = qcom_icc_rpm_set(src_qn, src_qn->sum_avg,
			       qp->ignore_enxio, src_qn->app_sum_avg);
	if (ret)
		return ret;
	...
	if (dst) {
		...
		ret = qcom_icc_rpm_set(dst_qn, dst_qn->sum_avg,
				       qp->ignore_enxio, dst_qn->app_sum_avg);
		if (ret)
			return ret;
	}
